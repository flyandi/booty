...
abstract class BaseQuery implements IteratorAggregate {
	...
	protected function __construct(BootyPDO $fpdo, $clauses) {
		$this->fpdo = $fpdo;
	...
	protected function resetClause($clause) {
		// BootyPDO changes - map clauses to others
		$clause = $this->map($clause);
	...
	/** 
	  * BootyPDO Extension Map
	  */

	private function map($clause) {
		// create map definitions
		$map = array(
			"CREATE" => "INSERT INTO",
		);

		// check
		return isset($map[$clause]) ? $map[$clause] : $clause;
	}
